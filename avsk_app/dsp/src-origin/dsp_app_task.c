...
//	{120-24, 120, 80, 240, 0, 0 },	//car box
	{320, 0, 736-320, 400, 90, 60 },	//single camera view
...
//		前摄像头全屏显示
		...
		//自定义界面，400*80
		win_startX[4] = 320,win_startY[4] = 0,win_Width[4] = 132,win_Height[4] = 60;
			crop_startX[4] = 0, crop_startY[4] = 0;

		win_startX[5] = 320,win_startY[5] = 61, win_Width[5] = 736 - 320,win_Height[5] = 7;
		crop_startX[5] = 0, crop_startY[5] = 0;

		win_startX[6] = 320 + 50,win_startY[6] = 100,win_Width[6] = 322,win_Height[6] = 48;
		crop_startX[6] = 0, crop_startY[6] = 0;

		win_startX[7] = 320 + 50,win_startY[7] = 170,win_Width[7] = 322,win_Height[7] = 48;
		crop_startX[7] = 0, crop_startY[7] = 0;

		win_startX[8] = 320 + 50,win_startY[8] = 240,win_Width[8] = 322,win_Height[8] = 48;
		crop_startX[8] = 0, crop_startY[8] = 0;
//		//背景
		win_startX[9] = 320,win_startY[9] = 0,win_Width[9] = 736 - 320,win_Height[9] = 480;
...
//全景+前视
		...
		crop_startX[2] = 0, crop_startY[2] = 0;
		//OSD
		win_startX[3] = 320, win_startY[3] = 400,win_Width[3] = 480,win_Height[3] = 80;
...
/*
 *            if(1 == tUIFsm.u8ReqTakeSnapshot)
            {
                pDSP_To_A8_TempCmdMsg_intask->cmdType = 0x555;
                tUIFsm.u8ReqTakeSnapshot = 0;
            }
*/
...
//            Vps_rprintf("[DSP] %d, Send video to A8 ,cmd = 0x777, pointer = %x\n", mySentToA8Counter, mutualTaskCmdMsg.pY_Pointer0);

            /*此处清空，会不会待A8提取时也会变成无效数据？不过目前版本并未支持视频存储，暂不考虑*/
            mutualTaskCmdMsg.cmdType = 0;
            mutualTaskCmdMsg.pY_Pointer0 = NULL;

        } // if(((mutualTaskCmdMsg.cmdType == 0x777)...
        else {
        	Task_sleep(1u); //1u
        }
...
//		if( ((pFrom_A8_TempCmdMsg_intask->cmdType >> 16) != 0x666) &&
//				((pFrom_A8_TempCmdMsg_intask->cmdType >> 16) != 0x555) )//liuxu, 06/19/2014, RISK...Be careful for rev mapping.
//		{
...
//		}
		...
		/*拍照执行结果*/
		if((pFrom_A8_TempCmdMsg_intask->cmdType >> 16) == 0x555) {
			u8 ret = (pFrom_A8_TempCmdMsg_intask->cmdType & 0xFFFF);//liuxu, 06/19/2014, add info channel among of A8 to DSP/M3.
			if(ret == MSG_SNAPSHOT_OK) {
				tUIFsm.bRet_TakenSnapshot = RET_OK;
			}else {
				tUIFsm.bRet_TakenSnapshot = RET_FAIL;
			}
		}
...
//		Vps_rprintf("[DSP] >>>>>>>>>>>>> %d, Send a video message, cmd = 0x777, mutualTaskCmdMsg.pY_Pointer0 = %x\n", s_cunt++, mutualTaskCmdMsg.pY_Pointer0);
#endif
...
{	
	 ...
	 */
	ti_srv_control_static(GA_S_FOCAL_LENGTH, 247, NULL ); //1.58mm/6.35um = 248
//	ti_srv_control_static(GA_S_FOCAL_LENGTH, 248, NULL ); //1.58mm/6.35um = 248
	/*
	 * 设置混合区域，单位为像素
...
#endif
//
//            	Vps_rprintf("[DSP] ====== Starting Colabration, using defualt persmat.dat ======\n");
//            	inpersmatPtr = persmatin;

        		*(unsigned int*)0x80000888 = 0x0;	/*清除标定标志*/
        		tUIFsm.bRet_Colabration = RET_UNKOWN; 	/*清除标定结果*/
            }
            	...
            	totalTicks2 += (profiletick4-profiletick3);
                 ...
                 if(triggerGA)
                 {
             		...
             		if(flagGASuccess == 1) {
             			...
             			Vps_rprintf("[DSP] Colabrate OK! \n");
             		}else{
             			tUIFsm.bRet_Colabration = RET_FAIL;
             			Vps_rprintf("[DSP] Colabrate Failed! \n");
             		}
            ...
            if(flagGASuccess == 1) {
            	...
            	/*
            	 * 标定成功后，将原SD卡中的Persmat.dat保存到FLASH中。界面上，显示标定成功提示图标
            	 */
//            	tUIFsm.bRet_Colabration = RET_OK; 	/*设置标定成功*/
            }
...
//static Word32 persmatin[] = {1168548, 4691, -29928995, 7274, 2387337, -4112491, -9513, 325, 1048576, 3430890, 83407, 69132337, -221228, 3112890, 115834630, 1160, 2661, 1048576, 1465549, 2723, 51888110, 21501, 2237038, -881129, 7046, -332, 1048576, 3454702, -295296, 104147669, 99852, 2952685, -114814057, 600, -2875, 1048576};
Word32 persmatin[] = {1131601, 81042, 7261885, -38166, 1464216, -22484589, -2277, 823, 1048576, 1782065,
		-118325, -61070805, -39328, 1583282, 109851828, 717, -707, 1048576, 962336, -67172, 1721768, 24735,
		1384077, -6917033, 3589, 251, 1048576, 1580675, 171120, -27832885, -36117, 1451835, -65973417, 779,
		-513, 1048576};
...
//            	Vps_rprintf("[DSP] Use default Persmat.dat\n");
//            	//============gzd ,test
//            	inpersmatPtr = persmatin;
            ...
            //int ti_srv_process(Word32 *inpersmatPtr, int *flagGASuccess)
            {
            	...
            	totalTicks2 += (profiletick4-profiletick3);
                 ...
                 if(triggerGA)
                 {
            		...
            		/*返回标定结果状态*/
            		if(flagGASuccess == 1) {
                     	Vps_rprintf("\n[DSP] ============ Colabration Finished! ========== \n");
                     	tUIFsm.bRet_Colabration = RET_OK;
					 }else {
						Vps_rprintf("\n[DSP] ============ Colabration err! flagGASuccess = %d, ========== \n", flagGASuccess);
                     	tUIFsm.bRet_Colabration = RET_FAIL;
					 }
            	}
