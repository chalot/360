...
void edmaWarpImgCpy5(EDMA3_DRV_Handle hEdma, void *srcBuf, void *dstBuf, Uint32 width, Uint32 height, Uint32 user_srcbidx, Uint32 user_desbidx)//liuxu, 02/13/2014.
{
    EDMA3_DRV_Result result = (int32_t)EDMA3_DRV_SOK;
    EDMA3_DRV_PaRAMRegs paramSet = {0u,0u,0u,0u,0u,0,0,0u,0u,0,0,0u,0};
    /* EDMA3_DRV_SyncType syncType; */
    ...
    paramSet.srcBIdx    = -2; //(int16_t)srcbidx;
    paramSet.destBIdx   = (int16_t)desbidx;
    paramSet.srcCIdx    = srcbidx; //(int16_t)srccidx;
    paramSet.destCIdx   = 2;//(int16_t)descidx;


    paramSet.aCnt       = 2; //(uint16_t)acnt;
    paramSet.bCnt       = bcnt; // acnt /2; //(uint16_t)bcnt;
    paramSet.cCnt       = acnt/2 ; //bcnt; //(uint16_t)ccnt;

    /* For AB-synchronized transfers, BCNTRLD is not used. */
    paramSet.bCntReload = 0; //(uint16_t)bcnt;
    ...
    //if(cccc % 100 == 0)
    {
   /* 	Vps_rprintf("[EDMA params ][%x,%x],[%x,%x,%x,%x],[%x,%x,%x],[%x,%x],[%x]\n" ,
    			paramSet.srcAddr,  paramSet.destAddr  ,
    			  paramSet.srcBIdx   , paramSet.destBIdx  ,  paramSet.srcCIdx   ,  paramSet.destCIdx,
    			  paramSet.aCnt , paramSet.bCnt , paramSet.cCnt,
    			  paramSet.bCntReload,paramSet.linkAddr ,
    			  paramSet.opt
    			);
    */
    }
...
void edmaWarpImgCpy6(EDMA3_DRV_Handle hEdma, void *srcBuf, void *dstBuf, Uint32 width, Uint32 height, Uint32 user_srcbidx, Uint32 user_desbidx)//liuxu, 02/13/2014.
{
    EDMA3_DRV_Result result = (int32_t)EDMA3_DRV_SOK;
    EDMA3_DRV_PaRAMRegs paramSet = {0u,0u,0u,0u,0u,0,0,0u,0u,0,0,0u,0};
    /* EDMA3_DRV_SyncType syncType; */
    ...
    paramSet.srcBIdx    = 2; //(int16_t)srcbidx;
    paramSet.destBIdx   = (int16_t)desbidx;
    paramSet.srcCIdx    = -1 * srcbidx; //(int16_t)srccidx;
    paramSet.destCIdx   = 2;//(int16_t)descidx;
...
//reverse mirror
void edmaWarpImgCpy7(EDMA3_DRV_Handle hEdma, void *srcBuf, void *dstBuf, Uint32 width, Uint32 height, Uint32 user_srcbidx, Uint32 user_desbidx)//liuxu, 02/13/2014.

{
    ...
    paramSet.destBIdx   = (int16_t)2;

    paramSet.srcCIdx    = srcbidx; //(int16_t)srccidx;

    paramSet.destCIdx   = srcbidx;//(int16_t)descidx;
    ...
    paramSet.bCnt       = acnt/2-1; // acnt /2; //(uint16_t)bcnt;

    paramSet.cCnt       = bcnt ; //bcnt; //(uint16_t)ccnt;
