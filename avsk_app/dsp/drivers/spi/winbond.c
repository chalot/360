...
//#include <common.h>//liuxu, 06/18/2014
//#include <malloc.h>//liuxu, 06/18/2014
#include "spi_flash.h"//liuxu, 06/18/2014

#include "spi_flash_internal.h"

#define ARRAY_SIZE(x) (sizeof(x) / sizeof((x)[0]))//liuxu, 06/18/2014
#define u16 unsigned short//liuxu, 06/18/2014.
#define u8 unsigned char//liuxu, 06/18/2014.
#define u32 unsigned int//liuxu, 06/18/2014.
#define size_t unsigned long//liuxu, 06/18/2014.
...
#ifdef CONFIG_TI811X_FAST_SPI_BOOT
extern int use_no_malloc;
#endif
...
#define offsetof(TYPE, MEMBER) ((unsigned int) &((TYPE *)0)->MEMBER)//liuxu, 06/18/2014.

#define container_of(ptr, type, member) ({			\
	const typeof( ((type *)0)->member ) *__mptr = (ptr);	\
	(type *)( (char *)__mptr - offsetof(type,member) );})//liuxu, 06/18/2014.
...
static int winbond_wait_ready(struct spi_flash *flash, unsigned long timeout)
{
	...
	} while (1);//liuxu, 06/18/2014, don't support get_timer. get_timer(timebase) < timeout
...
int winbond_read_fast(struct spi_flash *flash,
		u32 offset, size_t len, void *buf)
{
	...
	if(!use_no_malloc) 
#endif
		 stm = to_winbond_spi_flash(flash);
	u8 cmd[5];

	cmd[0] = CMD_READ_ARRAY_FAST;
#ifdef CONFIG_TI811X_FAST_SPI_BOOT
	if(use_no_malloc)
		winbond_build_address(NULL, cmd + 1, offset);
	else
#endif
...
#define min(X, Y)				\
	({ typeof (X) __x = (X), __y = (Y);	\
		(__x < __y) ? __x : __y; })//liuxu, 06/18/2014.
...
struct spi_flash *spi_flash_probe_winbond(struct spi_slave *spi, u8 *idcode)
{
	...
	if(use_no_malloc)
	{
		params = (void *)0x83000020;
		params = &winbond_spi_flash_table[3];
	}
	else
#endif
	{
