...
#define FVID2_SOK                       ((Int32) 0)//#include "vps_utils.h"//liuxu, 04/22/2014, porting from m3/hdvpss.

#define ENABLE_STATIC_ALLOCATION

#define REMOTE_DEBUG_SERVER_CURRENT_CORE         REMOTE_DEBUG_CORE_ID_C6xDSP//liuxu, 04/23/2014, fix bug of porting M3 core.
...
STATIC RemoteDebug_CoreObj *gRemoteDebug_coreObj = &gRemoteDebug_coreObjMem[0u];
...
#ifdef ENABLE_STATIC_ALLOCATION
    /* DONT DO ANYTHING - This allows remote prints/scanfs to continue till
     * the very end!! */
#else
    if (NULL != gRemoteDebug_coreObj)
    {
...
*/
int Vps_printf( xdc_String fmt, ... )//liuxu, 04/23/2014, orginal porting doesn't work.(char * format, ... )
{
    int retVal;
    ...
    cookie = Hwi_disable (  );//liuxu, 04/22/2014, RISK...??

    buf = &gRemoteDebug_serverObj.printBuf[0];

    va_start(vaArgPtr, fmt);
    vsnprintf(buf, REMOTE_DEBUG_SERVER_PRINT_BUF_LEN, fmt, vaArgPtr);
    va_end(vaArgPtr);
